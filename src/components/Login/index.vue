<template>
    <div class="login_body">
        ...
        <div>
            <input type="text" class="login_text" v-model="verifyImg" placeHolder="请输入您的验证码"> <img @click="handleToVerifyImg" src="/api2/users/verifyImg">
        </div>
        <div class="login_btn">
...
export default {
    ...
    methods : {
        handleToLogin(){
            this.axios.post('/api2/users/login',{      //向后台接口发送请求 
        ...
        handleToVerifyImg(ev){  //当点击这个验证码时，更新验证码
            ev.target.src = '/api2/users/verifyImg?'+Math.random(); //url要有变化它才会更新,所以加上Math.random()
            //ev.target 代表这张图片的DOM结构 
        }
    }
}
</script>
